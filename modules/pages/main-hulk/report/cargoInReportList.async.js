...
app.register.controller('cargoInReportList', function ($scope, $uibModal, $log, $http, $injector, $rootScope, listService, domainConfig) {

    var pager = listService.pager;

    $scope.cargoInReportVoList = [];
    ...
    $scope.queryCargoInReportList = function () {
        var params = {
            inDateStr: $scope.queryParameters.inDateStr,
        ...
        var url = domainConfig.hulkDomain + 'hulk/reportOutIn/cargoInReport';
        listService.events(url, params).success(function (data, status, headers) {
            $scope.cargoInReportVoList = data.cargoInReportVoList;
        }).error(function (data) {
            // $scope.cargoInReportVoList = [{
            //     batchNo: 1,
